...
$pageCategory = 'elderly';
$pageFile     = 'elderly.php';
...
<head>
    ...
    <title data-t="elder">Elderly - GS System</title>
...
<div id="deleteModal">
    ...
    <div class="container animate-fade" style="max-width: 1400px;">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:0.5rem; flex-wrap:wrap; gap:0.75rem;">
            <div>
                <h3 data-t="elder">Elderly Management</h3>
            ...
            <div style="display:flex; gap:0.75rem; align-items:center; flex-wrap:wrap;">
                <a href="add-elderly.php" class="btn btn-primary" style="text-decoration:none;">
                    <i class="fas fa-plus"></i> Add Elderly Entry
                </a>
        ...
        <div id="uploadSection" style="display:none; margin-bottom: 1.5rem;">
            <div class="glass" style="padding: 1.5rem; border-radius: var(--radius-lg);">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;">
                    <h4 style="color: var(--primary);"><i class="fas fa-file-excel"></i> Upload Excel (Elderly Template)</h4>
                    <button onclick="downloadElderlyTemplate()" class="btn" style="background:#e2e8f0; font-size:0.8rem; color:var(--text-dark); padding:0.4rem 0.8rem;">
                        <i class="fas fa-download"></i> Download Template
                    </button>
                </div>
                <div id="elderlyDrop" style="border: 2px dashed #cbd5e1; border-radius: 8px; padding: 2rem; text-align: center; cursor: pointer; background: rgba(255,255,255,0.5);">
                    ...
                    <p style="font-size: 0.75rem; color: var(--text-muted);">Columns: SNo, Name, House No, Address, Sex, Family Member, NIC No, Date of Birth, Age, Occupation, Conduct No, Person’s House No, Aswesuma, Elder</p>
                    <input type="file" id="elderlyFile" hidden accept=".xlsx,.xls">
                </div>
                <div id="elderlyUploadResult" style="display:none; margin-top:1rem; padding:1rem; background:#f1f5f9; border-radius:8px;"></div>
            </div>
        </div>
                ...
                <table style="font-size: 0.85rem; width: 100%; min-width: 1600px;">
                    ...
                    <tbody>
                        <?php if (empty($filteredResults)): ?>
                            <tr>
                                <td colspan="16" style="text-align: center; padding: 2rem; color: var(--text-muted);">No records found.</td>
                            </tr>
                        <?php else: ?>
                            <?php foreach ($filteredResults as $row): ?>
                                <tr>
                                    ...
                                    <td style="text-align: center;">
                                        <?php if ($row['member_count'] !== null): ?>
                                            <span class="badge badge-yes"><?php echo (int)$row['member_count']; ?></span>
                                        <?php else: ?>
    ...
    <script>
        ...
        const dropZ = document.getElementById('elderlyDrop');
        const fileIn = document.getElementById('elderlyFile');
        if (dropZ) {
            ...
            dropZ.ondrop = e => { e.preventDefault(); dropZ.style.borderColor=''; handleElderlyFile(e.dataTransfer.files[0]); };
            fileIn.onchange = e => handleElderlyFile(e.target.files[0]);
        }
        function handleElderlyFile(file) {
            if (!file) return;
            const reader = new FileReader();
            reader.onload = e => {
                const wb = XLSX.read(new Uint8Array(e.target.result), { type: 'array' });
                const json = XLSX.utils.sheet_to_json(wb.Sheets[wb.SheetNames[0]]);
                uploadElderlyData(json);
            };
            reader.readAsArrayBuffer(file);
        }
        async function uploadElderlyData(rows) {
            const res = document.getElementById('elderlyUploadResult');
            res.style.display = 'block';
            res.innerHTML = '<p>Uploading...</p>';
            let ok = 0, err = 0, errs = [];
            for (let i = 0; i < rows.length; i++) {
                const r = await fetch('api/elderly-upload-process.php', {
                    method: 'POST',
        ...
        function downloadElderlyTemplate() {
            const t = [{ 'SNo': 1, 'Name': 'Elder Name', 'House No': '201', 'Address': 'Hospital Road', 'Sex': 'M', 'Family Member': 4, 'NIC No': '195012345678', 'Date of Birth': '1950-01-01', 'Age': 73, 'Occupation': 'Retired', 'Conduct No': '0775556667', 'Person’s House No': '182A/3/2', 'Aswesuma': 'Severely Poor', 'Elder': 5000 }];
            const ws = XLSX.utils.json_to_sheet(t);
            const wb = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(wb, ws, 'Elderly');
            XLSX.writeFile(wb, 'Elderly_Template.xlsx');
        }
        // Bulk Actions Logic
