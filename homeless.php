...
<head>
    ...
    <title>House Needs - GS System</title>
...
<div id="deleteModal">
    ...
    <nav class="glass-dark" style="padding: 1rem 2rem; display: flex; justify-content: space-between; align-items: center;">
        <div style="display: flex; align-items: center; gap: 1rem;">
            ...
            <h2>House Needs (Housing Assistance)</h2>
        </div>
    ...
    <div class="container animate-fade" style="max-width: 1400px;">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:0.5rem; flex-wrap:wrap; gap:0.75rem;">
            ...
            <div style="display:flex; gap:0.75rem; align-items:center; flex-wrap:wrap;">
                <a href="add-homeless.php" class="btn btn-primary" style="text-decoration:none;">
        ...
        <div id="uploadSection" style="display:none; margin-bottom: 1.5rem;">
            <div class="glass" style="padding: 1.5rem; border-radius: var(--radius-lg);">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;">
                    <h4 style="color: var(--primary);"><i class="fas fa-file-excel"></i> Upload Excel (Homeless Template)</h4>
                    <button onclick="downloadHomelessTemplate()" class="btn" style="background:#e2e8f0; font-size:0.8rem; color:var(--text-dark); padding:0.4rem 0.8rem;">
                        <i class="fas fa-download"></i> Download Template
                    </button>
                </div>
                <div id="homelessDrop" style="border: 2px dashed #cbd5e1; border-radius: 8px; padding: 2rem; text-align: center; cursor: pointer; background: rgba(255,255,255,0.5);">
                    ...
                    <p style="font-size: 0.75rem; color: var(--text-muted);">Columns: SNo, Name, House No, Road, Family No, NIC No, Occupation, Phone No, Situation, Aswesuma, Remarks</p>
                    <input type="file" id="homelessFile" hidden accept=".xlsx,.xls">
                </div>
                <div id="homelessUploadResult" style="display:none; margin-top:1rem; padding:1rem; background:#f1f5f9; border-radius:8px;"></div>
            </div>
        </div>
        <div class="glass" style="padding: 2rem;">
            <div class="table-wrapper">
                <table style="font-size: 0.85rem; width: 100%; min-width: 1500px;">
                    <thead>
                        <tr>
                        ...
                        <?php else: ?>
                            <?php foreach ($filteredResults as $row): ?>
                                <tr>
                                    ...
                                    <td><?php echo htmlspecialchars($row['housing_condition'] ?? '-'); ?></td>
    ...
    <script>
        ...
        const dropZ = document.getElementById('homelessDrop');
        const fileIn = document.getElementById('homelessFile');
        if (dropZ) {
            ...
            dropZ.ondrop = e => { e.preventDefault(); dropZ.style.borderColor=''; handleHomelessFile(e.dataTransfer.files[0]); };
            fileIn.onchange = e => handleHomelessFile(e.target.files[0]);
        }
        function handleHomelessFile(file) {
            if (!file) return;
            const reader = new FileReader();
            reader.onload = e => {
                const wb = XLSX.read(new Uint8Array(e.target.result), { type: 'array' });
                const json = XLSX.utils.sheet_to_json(wb.Sheets[wb.SheetNames[0]]);
                uploadHomelessData(json);
            };
            reader.readAsArrayBuffer(file);
        }
        async function uploadHomelessData(rows) {
            const res = document.getElementById('homelessUploadResult');
            res.style.display = 'block';
            let ok = 0, err = 0, errs = [];
            for (let i = 0; i < rows.length; i++) {
                const r = await fetch('api/homeless-upload-process.php', {
                    method: 'POST',
        ...
        function downloadHomelessTemplate() {
            const t = [{ 'SNo': 1, 'Name': 'Sunil Perera', 'House No': '123', 'Road': 'Main Street', 'Family No': 'FAM-001', 'NIC No': '198012345678', 'Occupation': 'Farmer', 'Phone No': '0771234567', 'Situation': 'Needs permanent housing', 'Aswesuma': 'Poor', 'Remarks': '' }];
            const ws = XLSX.utils.json_to_sheet(t);
            const wb = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(wb, ws, 'Homeless');
            XLSX.writeFile(wb, 'Homeless_Template.xlsx');
        }
        // Bulk Actions Logic
